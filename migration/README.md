...
| 種別 | セレクト | `入庫` / `通常販売` / `関係者価格販売` / `プレゼント` / `拠点間移動` / `卸し` / `棚卸調整` の7択。このスクリプトは常に `通常販売` を書き込みます |
| 日時 | 日付 | 取引が発生した日時 |
| 拠点 | セレクト | `水上村` / `町田寮`。Wixの注文は全件 `水上村` から発送された扱いで登録します（`--location` で変更可） |
| 移動先拠点 | セレクト | `水上村` / `町田寮`。拠点間移動のときのみ使用（このスクリプトでは書き込みません） |
| 商品名 | テキスト | 商品名 |
| 数量 | 数値 | 購入数量 |
| 単価 | 数値 | 商品単価（税込） |
...
Wix以外の拠点（町田寮など）から出荷したデータを移行する場合は `--location 町田寮` を指定してください（未指定時は `水上村`）。
