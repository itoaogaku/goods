...
export function WixSyncButton({ onSynced }: WixSyncButtonProps) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      const parts = [`新規${data.created}件`, `ステータス更新${data.statusUpdated}件`];
      if (data.nameBackfilled > 0) parts.push(`顧客名補完${data.nameBackfilled}件`);
      parts.push(`変更なし${data.skipped}件`);
