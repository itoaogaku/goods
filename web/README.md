...
`ステータス`（未発送/発送済/キャンセル/返金）は、**Wix経由の注文についてはWixのfulfillmentStatus（発送状況）をそのままAPI経由で取り込みます**。Wix上で発送済みにマークすると、次回の同期（最大30分後、または手動同期ボタン）でこのアプリ側のステータスも自動的に更新されます。キャンセル・返金はWixの決済ステータスから判定し、発送状況より優先されます。
...
- `WIX_SYNC_SINCE`（未設定時は`SALES_DATA_SINCE`＝2025年3月1日）以降の注文は毎回すべて取得対象になるため、**初回同期で過去分も含めて自動的に取り込まれます**（1回の同期で新規作成できる件数は最大200件のため、件数が多い場合は複数回の同期に分けて取り込まれます。「今すぐWixと同期」を何度か押すか、30分待てば自動的に完了します）。`../migration` のCLIはWixのAPIが使えない場合の代替手段（CSVエクスポートからの移行）として残していますが、通常は不要です。
